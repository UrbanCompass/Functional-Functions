...
try:
    ...
    print(
        """We couldn\'t find your settings file. \n
...
def load_via_sql_snowflake(
    ...
    load_df["etl_inserted_timestamp"] = pd.Timestamp.now(tz="UTC").tz_convert(
        "US/Eastern"
    )
    ...
    engine = create_engine(
        f"snowflake://gl11689.us-east-1.snowflakecomputing.com", creator=lambda: conn
    )
...
def load_via_sql_snowflake_agent_ar(
    load_df, tbl_name, if_exists="replace", test_mode=True, creds=None
):
    ...
    if if_exists == "replace":
        ...
        print(
            f"Since if_exists = replace; dropping the existing table and loading the data: {tbl_name}"
        )

        load_df.write.format("snowflake").options(**creds_dict).option(
            "dbtable", tbl_name
        ).mode("overwrite").save()
        ...
        print(
            f"Since if_exists = append; appending the data to the existing table: {tbl_name}"
        )
    ...
    if os.environ.get("environment") == "databricks":
        ...
        if sys.platform == "win32":
            ...
            try:
                ...
                print(
                    "could not find or create logs dir, please create manually or double check permission"
                )
                print(str(e))
        else:
            ...
            try:
                if path.exists(full_path):
                    ...
                    # print('You don\'t seem to have a logs file, what a shame. I\'ll go ahead and make one for you')
                logging.basicConfig(
                    filename=full_path + "/logger_" + str(dt.now()) + ".log",
...
def query_databricks(query, full_table=False, use_service_account=False):
    ...
    if use_service_account == True or os.environ.get("environment") == "databricks":
        ...
        dbx_sql = DBX_sql(
            server_hostname=dbx_host, http_path=dbx_path, access_token=dbx_token
        )
    else:
        dbx_sql = DBX_sql()
...
def save_pickle(df, file_name, folder_name=None, file_path_option=None):
    ...
    fp += (
        folder_name + file_name + "_" + str(dt.today().date()).replace("-", "_") + ".p"
    )
...
def batch_start(test_mode, script_name, creds=None):
    ...
    q = """INSERT INTO fpa_sandbox.batch_table (batch_status,start_time,batch_hash,test_mode,script_name, user_running) 
        ...
        dt_now, hash_id, test_mode, script_name, user_running
    )
...
def batch_update(status, hash_id, creds=None):
    ...
    q = "UPDATE fpa_sandbox.batch_table SET batch_status = '{}', end_time = CURRENT_TIMESTAMP() WHERE batch_hash = '{}' ".format(
        status, hash_id
    )
...
def load_via_sql_dbx(
    load_df, tbl_name, local_path="./data", test_mode=True, s3_file=None
):
    """
...
def query_method_by_env_dbx(query, use_service_account=True):
    ...
    if os.environ.get("environment") == "databricks":
        ...
        if use_service_account == True:
            logging.debug("query via sql endpoint, using service account creds")
        else:
            logging.debug("query via sql endpoint, using personal account")
...
def get_spark_schema(pdf):
    ...
    for col, typ in zip(list(pdf.columns), list(pdf.infer_objects().dtypes)):
        ...
        if typ == "object":
            spark_typ = StringType()
        elif typ == "int64":
            spark_typ = IntegerType()
        elif typ == "datetime64[ns]" or typ == "datetime64":
            spark_typ = TimestampType()
        else:
            spark_typ = DoubleType()

        if (col.lower().endswith("date") or col.lower().endswith("period")) and (
            col.lower() not in [x.lower() for x in datetime64_cols]
        ):
            spark_typ = DateType()
        if col == "TRANSACTION_ID":
            spark_typ = IntegerType()
        if col == "CLEAN_ADDRESS_PARSE":
            spark_typ = BooleanType()
        if col == "same_market" or col == "journal_entry_flag":
            spark_typ = BooleanType()
